---
name: Agenda
description: Specifies activation ordering and lifecycle.
...
- Creation: When a token reaches a production node, create an activation `{rule_id, token_signature, time}`.
- De-dup: If an identical activation exists, update recency but avoid duplicates.
- Cancellation: Retracts remove corresponding activations.
