---
name: Tool Calling
description: [ツールの呼び出し](/docs/modules/model_io/chat/function_calling)により、モデルはいつ1つ以上のツールを呼び出す必要があるかを検出し、それらのツールに渡すべき入力を返すことができます。APIコールでは、ツールを記述し、それらのツールを呼び出すための引数を
