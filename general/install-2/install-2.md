...
# 使用内置精选提示词
python z_image_modelscope.py --loops 5 --no-prompt-api
```

### GGUF 量化模型（z_image_window.py）

```bash
# Windows 低显存模式
python z_image_window.py --gguf-model "models\z_image_turbo-Q3_K_M.gguf" --prompt "A cat"
```
...
脚本已内置 Flash Attention 2/3 支持，会自动检测并启用最佳后端。
...
| 包 | 版本 | 说明 |
|---|---|---|
| numpy | 1.26.4 | 避免与 SciPy ABI 不兼容 |
| scipy | 1.12.0 | 与 NumPy 1.26.4 完全兼容 |
| torch | >=2.1.0 | 支持 CUDA 12.1 |
| diffusers | >=0.27.0 | Stable Diffusion 管道 |
| transformers | >=4.30.0 | Hugging Face Transformers |
| accelerate | >=0.20.0 | 分布式训练加速 |
| modelscope | >=1.10.0 | ModelScope SDK |
| Pillow | >=10.0.0 | 图像处理 |
...
```bash
# 查看完整参数
python z_image.py --help

# 查看日志
tail -f zimage_generation.log
```