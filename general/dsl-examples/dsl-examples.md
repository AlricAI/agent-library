---
name: Dsl Examples
description: Illustrative rules spanning multiple domains and layered scopes.
...
## Weekly overtime (general + tenant override)
...
rule "nurse-min-rest-between-shifts" salience: 65 do
  when
    ...
    guard s > f and hours_between(f, s) < 8
  then
    emit ComplianceViolation(employee_id: e, period_key: to_day(s), code: "REST_SHORTFALL", severity: :high, details: "Less than 8h between shifts")
end
```
