---
name: Refraction
description: Prevents rules from firing repeatedly on identical token combinations.
...
- Bounded Storage: Store signatures in partition-local ETS with eviction policy (TTL/LRU). Default: per-production exact set with 24h TTL; cap by bytes per partition.
