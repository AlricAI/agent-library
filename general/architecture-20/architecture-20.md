---
name: Architecture
description: This document outlines a practical architecture for implementing a RETE engine in Elixir.
...
- Agenda: Conflict set of activations with pluggable policies (salience, recency, specificity) and refraction controls.
- RHS Executor: Executes rule actions (emit derived facts, callbacks) under the Engine’s supervision and boundaries.
- Phoenix UI: LiveView interface for rules authoring, validation, and activation workflows.
- gRPC Server: External API for engine operations and ruleset management.

## Rulesets and Compilation

- Ruleset Repository: A per-tenant ruleset store with support for shared rule bundles and tenant-specific overlays. Each ruleset has an immutable version once compiled.
- Precompilation: On rule creation/validation, compile DSL to a network and specialized predicate modules. Store artifacts with a semantic `ruleset_version`.
- Dynamic Activation: Engines accept `set_ruleset/2` to atomically swap to a compiled ruleset version; facts specify `ruleset_version` to ensure alignment during ingest.
- Shared + Overlay Model: Compose a tenant ruleset from a base shared bundle plus a tenant overlay; the compiler produces a single merged network for activation.
- Hot Reload Safety: Activation occurs at agenda-safe boundaries; in-flight batches finish under the previous ruleset.

## Multi-Tenancy and Partitioning

- Engine-per-Tenant: Run one logical engine instance per tenant (keyed by `tenant_id :: String.t()`) to isolate working memory and agendas.
  Target 1000 concurrent engines across a cluster; per-node concurrency depends on tenant size mix.
- Internal Partitions: Within an engine, partition working memory and node state by a stable hash (e.g., 16–64 partitions). Each partition is managed by its own process to leverage BEAM schedulers.
- Static Network Sharing: The compiled network (nodes/edges and predicate code) is immutable and stored in `persistent_term` to be shared by all partitions within an engine.
- Co-location Strategy: Choose partition key to co-locate WMEs that frequently join (e.g., by `employee_id` or other dominant join key). Cross-partition joins are minimized;
  when unavoidable, use routed probes with bounded fan-out.

## Horizontal Scaling and Discovery

- Clustered Deployment: Run multiple Phoenix nodes behind a gRPC load balancer; engines are started on specific nodes.
- Tenant Routing: A cluster-aware registry maps `tenant_id` to the hosting node/engine; requests are forwarded or proxied appropriately.
- Engine Lifecycle: Idle eviction and on-demand starts reduce memory footprint for 1000-tenant fleets; cold start budget < 1s with precompiled artifacts.
...
```mermaid
flowchart TB
  PS[PartitionSupervisor Epona.Partitions]
  PS --> DS0[DynamicSupervisor p0]
  PS --> DS1[DynamicSupervisor p1]
  PS --> DSn[DynamicSupervisor pN]
  DS0 -->|start_child hash: tenant_id| Eng0[Epona.Engine tenant A]
  DS1 -->|start_child hash: tenant_id| Eng1[Epona.Engine tenant B]
```

- Single-Engine Simplicity: Start with one `GenServer` per engine instance for determinism and ease of reasoning. All mutations go through serialized handle calls/casts.
- Partitioned Supervision: All GenServers MUST be started via a `PartitionSupervisor` to reduce contention and align work by key.
  - Top-level: `{PartitionSupervisor, name: Epona.Partitions, partitions: 16, with: {DynamicSupervisor, strategy: :one_for_one}}` (partition count configurable).
  - Start children with `PartitionSupervisor.start_child/3`, passing `hash: key` (e.g., `tenant_id` or `{tenant_id, subkey}`) for stable placement.
  - No GenServer starts directly under the root supervisor; all dynamic children are started on a partition’s DynamicSupervisor via the `PartitionSupervisor`.
- Batching: Accept and process batches of deltas (assert/modify/retract) atomically to maintain consistent propagation and agenda snapshots.
- Backpressure: Limit in-flight batches; configurable mailbox limits with metrics.
- Parallelism: Process independent partitions concurrently. For cross-partition effects (rare), use message passing with correlation IDs and bounded degree.
- Orchestrator: A lightweight per-engine coordinator distributes batches to partitions, merges activation outputs, and enforces fairness.
- Ruleset Control Plane: A per-node service that loads/evicts compiled rulesets to/from memory (`persistent_term`/code server) and signals engines to switch.
