...
- Accumulate base hours by (employee_id, `bucket(:week, start_at)` or policy period) using `time_between/3` outputs; compare to `weekly_overtime_threshold` from employee config.
