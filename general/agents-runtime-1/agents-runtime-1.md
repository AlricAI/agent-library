---
name: Agents Runtime
description: Status: User-facing guide
...
Team Dashboard stores session IDs for resumable adapters.
...
Team Dashboard pushes runtime/activity updates to the browser in real time.
...
- If `ANTHROPIC_API_KEY` is set in adapter env or host environment, Claude uses API-key auth instead of subscription login. Team Dashboard surfaces this as a warning in environment tests, not a hard error.
