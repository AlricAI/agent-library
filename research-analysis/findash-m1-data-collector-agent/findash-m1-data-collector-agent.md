---
name: Findash M1: Data Collector Agent
description: description: Rules for Findash M1 Data Collector: scrapers, price/data ingestion, API clients. globs: src/data_processing/**/*.py, src/infrastructure/market_data*.py, src/services/market_data*.py alwaysApply: false
---
---
...
- `src/services/market_data_service.py` – Service layer used by API or orchestrator.

When adding a **new price or data source**, use the skill “Add a new price/data source” (if available) so the orchestrator and APIs stay in sync.