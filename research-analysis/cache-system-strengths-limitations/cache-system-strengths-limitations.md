---
name: Cache System Strengths Limitations
...
- **Channel alias reliance.** Runtime channel selectors depend on aliases embedded in the analysis
  payload; unknown aliases throw resolution errors, so imported material lacking alias metadata must
  fall back to numeric indices.【F:src/audio/features/channelResolution.ts†L54-L104】
...
1. **Declare feature requirements.** Register the element with `registerFeatureRequirements`,
   requesting the waveform feature once so the intent bus schedules min/max peak extraction for the
   bound track.【F:docs/audio/audio-cache-system.md†L261-L287】【F:docs/audio/audio-cache-system.md†L160-L166】
2. **Sample per frame and read both channels.** In `_buildRenderObjects`, call
   `getFeatureData(this, trackId, 'waveform', time)`. The returned `values` array interleaves channel
   min/max pairs, so index `[0, 1]` for the first channel and `[2, 3]` for the second.【F:src/audio/features/sceneApi.ts†L197-L278】【F:src/audio/features/tempoAlignedViewAdapter.ts†L188-L217】
3. **Render using min/max envelopes.** Build vertical bars or polylines from the interleaved values
   without resampling the audio buffer yourself. Channel aliases from the track metadata can drive UI
   labels when needed.【F:docs/audio/audio-cache-system.md†L120-L170】【F:src/audio/features/tempoAlignedViewAdapter.ts†L188-L217】
...
1. **Request spectrogram data once.** Register feature requirements for the spectrogram feature. The
   analyzer always produces every available channel, including mid/side splits when supported.【F:docs/audio/audio-cache-system.md†L101-L170】
2. **Sample with presentation options.** During render, call
   `getFeatureData(this, trackId, 'spectrogram', time, { smoothing: 2, interpolation: 'linear' })`
   (adjust options as needed). The returned `values` array contains every channel for the requested
   frame.【F:src/audio/features/sceneApi.ts†L197-L278】【F:docs/audio/concepts.md†L9-L36】
3. **Map frequency bins to visuals.** Use the track’s channel metadata (aliases, semantics) to decide
   which indices correspond to Mid vs Side, then transform the per-channel magnitudes into bars or
   heatmaps.【F:docs/audio/audio-cache-system.md†L101-L170】【F:src/audio/features/audioFeatureTypes.ts†L42-L88】
4. **Detect unsupported material.** If `getFeatureData` returns `null`, inspect the timeline’s cache
   status or diagnostics to confirm the track exposes multi-channel analysis; fall back to mono
   rendering or prompt the user to re-run analysis when aliases are missing.【F:docs/audio/audio-cache-system.md†L57-L86】【F:src/core/scene/elements/audioFeatureUtils.ts†L58-L176】