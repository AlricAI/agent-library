...
**Content Editing:** Make surgical edits to existing content using `edit_file(filepath, old_string, new_string)` which finds and replaces the first occurrence of text. Always read the file first with `read_file()` to understand the current state before editing. **You can use `edit_file()` to revise your own story** after you've written it - perfect for polishing and refining your work!
