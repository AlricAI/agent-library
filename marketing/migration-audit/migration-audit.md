...
description: Read-only audit of the current `sighthound-content-ops` app against `design-system/`.
...
9. **Visual regression tooling**: do we stand up Playwright screenshots before Phase 3, or go manual-QA? — **resolved (post-migration)**. Minimal Playwright suite landed at `tests/visual/` after the migration stabilised; covers `/design-system-preview` + `/login` as public-route baselines. Auth-gated canonical pages stay out of scope until a Supabase test user exists; storage-state pattern sketched in `tests/visual/README.md`. **Queued expansion** (once a test user lands): add 3–4 authed canonical pages — `/dashboard` (table/chrome), a blog detail page (`/blogs/[id]`) (detail drawer + inline status chips), and a view with the Ask AI panel open (primitives + blocker/quality/next-steps cards in live composition). Pick surfaces that exercise the primitives end-to-end so any token drift surfaces in one diff.
...
- **`design-system/uploads/*.pdf` Git LFS** — **parked**. The 10 MB blob sits in `main` history. Reviewed post-migration (April 2026) and decided to take no action: low value (GitHub's soft repo-size limit is far off; the PDF rarely changes), non-zero risk (the only meaningful fix is `git lfs migrate import` which rewrites history and requires a force-push to `main`). Re-open only if repo clone size or CI bandwidth becomes a real problem. Until then, this is a “do nothing” decision, not a queued ticket.
...
- **Sidebar navy reopening (§11.6 trigger met)** — the original §11.6 decision kept the sidebar white pending “Blurple everywhere else.” That condition is now met (Phase 4.2 chrome + Phase 4.4 all pages + Phase 4.5 AI + §20 Card/Badge primitives). **Not acting.** Low urgency, product decision not a visual-migration decision. When appetite exists, the right path is a small **exploratory branch** (not a commit on `main`) that A/B-tests the Redactor-style navy sidebar (`#1a1d38` surface, `text-blurple-100` nav labels, `text-white` active) against the current white-on-gray-200 sidebar. Compare across: (a) 1440×900 dashboard first-paint, (b) collapsed 72px icon-only state, (c) high-density list views where the sidebar sits next to `bg-[color:var(--sh-gray)]` content. Gate on user / stakeholder preference, not technical drift. If adopted, the change is single-file (`src/components/app-shell.tsx` sidebar `<aside>` + nav-item active/inactive classes); if rejected, the branch dies. Either way, document the decision back here so the §11.6 note finally closes.
- **Density modes (compact / comfortable)** — §7.6 noted the DS doesn't speak to density preferences (`useDensityPreference()` stays `compact` by default). Not a DS gap the migration needed to fill — the Content Relay tokens are density-neutral and work in either mode. Revisit **only if** (a) density becomes a first-class user preference stored on `profiles.ui_density` (currently localStorage-only per AGENTS.md UX Primitives Authority §13), (b) the product wants to ship a “comfortable” mode for non-admin / executive views, or (c) accessibility feedback surfaces a legibility complaint tied to the compact default. At that point the DS `--space-*` scale + the typography type scale should both be revisited for the comfortable variant. Until any of those three triggers fire, no action.
