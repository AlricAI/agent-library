---
name: STUDIO
description: > **Name:** Pixelpit (approved)
...
| **Your daily check-in** | Opus | That's you talking to me (/pixelpit) |
...
Dead games go to `pixelpit/graveyard/` with a postmortem.
...
├── EXTERNAL-CHANGES.md       # Changes outside pixelpit/
│
├── mayor/
...
│
└── web/                      # web/app/pixelpit symlink target
```

### Web Integration

Games publish to `web/app/pixelpit/` (max 11 folders as specified):
- `web/app/pixelpit/` — main studio page, game index
- `web/app/pixelpit/g1/` through `web/app/pixelpit/g10/` — up to 10 live games

**Middleware**: Add `/pixelpit` to bypass list in `web/middleware.ts`

## Persistent State (Supabase)

Following Amber's pattern, all studio state lives in a `pixelpit_state` table:

```sql
CREATE TABLE pixelpit_state (
...
**CRITICAL FOR HAIKU AGENTS** — See `pixelpit/TASKS.md` for full spec.
...
## Your Interface (/pixelpit)

When you invoke `/pixelpit`, I (your Partner In Crime) will:
...
- [ ] Create `pixelpit_state` table in Supabase
- [ ] Add `/pixelpit` to web/middleware.ts bypass
- [ ] Create web/app/pixelpit/page.tsx (studio index)
- [ ] Create /pixelpit subagent command
...
- **All code in pixelpit/** — nothing pollutes vibeceo
