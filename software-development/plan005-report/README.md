...
The primary goal was to refactor the project to use the latest versions of its dependencies, based on the user's request to use "pnpm
...
- **`pnpm run test:ci`**: Passed, but also raised a `WARNING` from Turborepo.
  ...
  - **Solution:** Modified the `test:ci` script in `apps/game/package.json` to `vites
