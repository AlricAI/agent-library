---
name: Database Repository Patterns
description: description: SQLite database patterns and repository design for Project globs: - "packages/core-*/**/*.repo.ts"
---
---
...
```bash
# Backup database
cp .project/db/project.db .project/db/project.db.backup

# Run migration
node packages/pm-scripts/run-migration.js

# If failed, restore backup
cp .project/db/project.db.backup .project/db/project.db
```
...
const fixtureDb = makeDbClient({
  dbPath: './__tests__/fixtures/test-project.db',
});
...
const dbClient = makeDbClient({ dbPath: './db/project.db' });
...
- `project-tdd-workflow.rules.mdc` - Integration testing with real database
- `project-domain-logic.rules.mdc` - Domain logic using repositories

---

**Remember:** SQLite is Project's source of truth. All motivation metrics flow through the database. Optimize queries, use indexes, and always test with real data.