...
### 6. Env Var Reuse
Are existing env vars from dispatcher/.env reused, not duplicated?
...
### 7. Rollback
Can this be rolled back instantly? Feature flag? Simple revert?
...
- `issues` (array): Each has `severity` ("high" | "medium" | "low"), `issue` (what's wrong), `suggestion` (how to fix)
- `approved` (boolean): true if score >= 9
- `summary` (string): 1-2 sentence overall assessment

## Scoring Guide
- **10**: Perfect plan. No issues found. Ready to execute.
- **9**: Minor suggestions only. Approved — execute with notes.
- **7-8**: Real issues found but fixable. Needs revision.
- **5-6**: Significant gaps. Multiple revisions needed.
- **1-4**: Fundamental problems. Wrong approach or missing core requirements.