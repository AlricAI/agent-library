---
name: Laravel 13 Backend Rules
description: globs: "**/*.php" Laravel 13 Backend Rules ## Laravel 13 Structure
---
---
globs: "**/*.php"
---
...
**Automatic Relationship Loading** (Laravel 12.8+):
```php
// Per-query — auto-loads relations as accessed
...
// Globally (AppServiceProvider)
Model::automaticallyEagerLoadRelationships();
```
Explicit `with()` still preferred for controller queries (predictable query count). Auto-loading is excellent for views and deeply nested access.
...
**UUIDv7 default**: `HasUuids` now generates UUIDv7 (time-ordered). For UUIDv4: `use HasVersion4Uuids as HasUuids;`
...
{
    return [
        'address' => AsFluent::class,              // JSON as Fluent object
        'homepage' => AsUri::class,                 // URI object casting
        'tags' => AsCollection::of(Tag::class),     // Typed collection items
        'data' => 'json:unicode',                   // Preserve Unicode
    ];
}
```
...
class Order extends Model {}  // No need to override newEloquentBuilder()
```

**Laravel 13**: Creating model instances inside `boot()` / `boot*()` methods now throws `LogicException`. Move instantiation outside the boot cycle.
...
**FailOnException middleware**: Immediately fail job on specific exceptions:
```php
public function middleware(): array
{
    return [new FailOnException(PaymentDeclinedException::class)];
}
```
...
```php
// config/queue.php
'failover' => ['driver' => 'failover', 'connections' => ['redis', 'sqs', 'database']],
```

**Batch/Chain**: `Bus::batch([...])->then(fn() => ...)->dispatch()` parallel; `Bus::chain([...])->dispatch()` sequential

**Laravel 13 queue changes**:
- `JobAttempted` event: `$exception` (Throwable|null) replaces `$exceptionOccurred` (bool)
- `QueueBusy` event: `$connectionName` replaces `$connection`
- Queued notifications respect `#[DeleteWhenMissingModels]` attribute
...
**Factory insert()**: `User::factory()->count(1000)->insert();` — mass insert, faster than `create()` for seeding
...
**Laravel 13**: Custom `Str` UUID/ULID/random factories reset between test methods. Set them in each test or `setUp()`.
...
- **Laravel 13**: Add `serializable_classes` to `config/cache.php` — set to `false` or explicit class allow-list to prevent deserialization attacks
...
**Laravel 13**: Cache `serializable_classes` config defaults to `false`. List allowed classes explicitly if caching PHP objects:
```php
'serializable_classes' => [App\Data\CachedStats::class],
```
...
- **Laravel 12 breaking**: Default local disk root changed from `storage/app` to `storage/app/private`
- **Laravel 12 breaking**: SVGs no longer pass `image` validation rule — use `'image:allow_svg'` or `File::image(allowSvg: true)`
- **Laravel 13 breaking**: CSRF middleware renamed from `VerifyCsrfToken` to `PreventRequestForgery`. Use `preventRequestForgery()` in bootstrap/app.php
- **Laravel 13 breaking**: `Inertia::lazy()` / `LazyProp` removed — use `Inertia::optional()` instead
- **Laravel 13 breaking**: Cache/Redis prefixes now hyphenated (`app-name-cache-`), session cookie uses `Str::snake()` + `_session`
- **Laravel 13 breaking**: Routes with explicit domain now prioritized before non-domain routes
- **Laravel 13 breaking**: Pagination Bootstrap 3 views renamed: `pagination::default` → `pagination::bootstrap-3`
- **Laravel 13 breaking**: `Js::from` uses `JSON_UNESCAPED_UNICODE` by default