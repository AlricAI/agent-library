---
name: MCP Tools Complete Reference
description: description: Complete reference for all MCP tools: CLI operations, API validation, documentation lookup, GitHub integration, file analysis, code investigation, memory management, task management. Use when: planning features, developing code, researching solutions, managing project knowledge, analyzi
---
---
...
- Workflow: Planning → Work transition, API documentation phase
...
1. **Error Investigation**: \`find_implementation\` → \`explore_source\` → \`analyze_file\`
2. **API Discovery**: \`find_implementation\` → \`explore_source\` with context
3. **Complex Tracing**: \`trace_dependency_chain\` → \`explore_source\` on final implementation
...
- Use when: Linking requirements→design→tasks, connecting patterns
...
6. **Assess delegation potential** - complex/independent tasks → delegate, integrated/strategic tasks → execute directly
...
- **Unknown symbols/imports**: \`find_implementation\` → \`explore_source\`
- **Complex dependencies**: \`trace_dependency_chain\`
- **API testing**: \`get_openapi_spec\` → \`make_api_request\`
- **Research**: \`search_github_code\` → \`get_documentation\`
- **Knowledge capture**: \`store_memory\` → \`relate_memories\`
- **Project management**: \`list_tasks\` → \`create_task\` → \`update_task_status\`
- **Workload distribution**: \`check_agent_environment\` → \`delegate_to_agent\` → \`monitor_agent_tasks\`
- **Strategic planning**: \`search_memories\` → \`create_task\` → \`assess delegation potential\`

### Delegation Decision Matrix
| Task Characteristics | Action | Tool Chain |
|---------------------|--------|------------|
| Complex + Independent | Delegate | \`check_agent_environment\` → \`delegate_to_agent\` → \`monitor_agent_tasks\` |
| Strategic + Integrated | Execute Directly | \`update_task_status\` → normal workflow |
