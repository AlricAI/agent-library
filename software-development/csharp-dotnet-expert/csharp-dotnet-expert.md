...
- **Null checks**: use `ArgumentNullException.ThrowIfNull(x, nameof(x))`; for strings use `ArgumentNullException.ThrowIfNullOrWhiteSpace(x, nameof(x))`; guard early. Avoid blanket `!`.
...
- Don't use **FluentAssertions/AwesomeAssertions**.
