---
name: AGENTS
description: This guide covers implementing cards and game mechanics in the server codebase.
...
-   The file name should match the card's ID from the JSON data
    -   Example: [BadOmen.js](game/cards/12-PV/BadOmen.js)
...
#### Querying Card JSON Data

**Correct jq usage** (use `.cards[]` to access the cards array):
...
-   Each new card implementation should also have corresponding tests as described in the [test AGENTS.md](../test/AGENTS.md).
...
-   After changing the behavior of the game engine, run all tests (not just the card's test) to ensure no regressions were introduced.
